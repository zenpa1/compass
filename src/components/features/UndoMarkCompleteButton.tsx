...
import { Work, markWorkAsNotComplete } 
    from "@/app/(dashboard)/projects/[projectId]/workDataOps";

interface UndoMarkCompleteButtonProps {
  label: string;
...
export function UndoMarkCompleteButton({
  label,
  tone,
  work,
  refresh
}: UndoMarkCompleteButtonProps) {
  const [showConfirm, setShowConfirm] = useState(false);

  const handleConfirm = () => {
    markWorkAsNotComplete(work.work_id);
    ...
    <>
      ...
      {showConfirm ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Mark work not complete confirmation"
        >
          <div className="w-full max-w-sm rounded-xl bg-white p-5 text-center shadow-lg">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Do you want to mark this work as NOT complete?
            </p>
